...
{
  int code = 0, j, n, initialruns, scheduledjobs;
  powerbag **ppbag = NULL, *pbag;
  double *matcopy = NULL, *scratch = NULL;
  double scale = 1.0;
  int quantity = 1, numworkers = 1, theworker,numeigenvalue = 1; 
  char gotone;
...
//read from file
PWRread(argv[1],&read_n,&read_matrix);     

  for(j = 0; j < numworkers; j++){
    //allocate memory and assign variables
    PWRallocateNload(read_n,j,read_matrix, ppbag + j,numeigenvalue);
  ...
  while(activeworkers > 0){
    ...
    /** if we are here, "theworker" can work **/
      pbag = ppbag[theworker];
      //printf("SCALEEEEEEEEEEEEEEEEEEEE is %g\n",scale);
      if((code = cheap_rank1perturb(n, pbag->scratch, pbag->matcopy, pbag->matrix, scale)))
	goto BACK;
  ...
  for(j = 0; j < numworkers; j++){
    ...
    pbag = ppbag[j];
    //
    for(int t=0;t<pbag->eigvaluecount;t++)
      printf("Eigenvalue %d  : %g\n",t,pbag->vectortopeigvalue[t]);
    PWRfreespace(&pbag);
  }
  free(ppbag);
