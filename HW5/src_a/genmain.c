...
int main(int argc, char **argv)
{
  ...
  if( (argc < 2) || (argc > 3)){
    printf("usage: factors filename [lambda]\n"); retcode = 1; goto BACK;
  }
  ...
  printf("numassets: %d; numfactors: %d; lambda: %g\n", numassets, numfactors, lambda);


  retcode = engine(numassets, numfactors, ub, lb, mu, sigma2, V, F, lambda);
